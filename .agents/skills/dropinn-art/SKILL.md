...
- For the interactive stage set, see [scene-stage-art.md](references/scene-stage-art.md). Run `node .agents/skills/dropinn-art/scripts/review-scenes.mjs` for landscape/portrait crops and 128/64/48px cutouts on light/dark backgrounds. The runtime uses lossless WebP derivatives; retain the original PNGs and verify exact RGBA equality when re-encoding.
