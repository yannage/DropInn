...
All three states reuse `public/art/ui-scroll-roller.webp`, a standalone roller generated specifically for slicing, with complete end caps and no dangling ribbons. `ScrollRoller` projects it through three SVG viewports: fixed end caps and a flexible middle. No alternate image is swapped when expanding. The source is retained in `ui-scroll-roller-source.png`; `.agents/skills/dropinn-art/scripts/prepare-scroll-roller.py` removes only alpha values at or below 16, crops with at least 16px clear margin, and creates the lossless runtime copy. Slice boundaries cross the shaft, never the outer silhouette. The paper overlaps within the opaque shaft to avoid transparent gaps. Earlier independent frame images remain as source history but are no longer used by this reader. A solid HTML parchment area contains the heading, controls, internally scrolling entries and New events button. Framer Motion animates dimensions over 350ms without scaling text. Reduced motion removes the transition. Buttons remain at least 44px; text enlargement reflows within the reader.
