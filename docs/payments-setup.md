...
`playdropinn.com` was submitted in **Checkout > Website approval** and is pending. A live API key named `DropInn live checkout service` was created with adjustment/discount/price/product read and transaction write permissions, a December 25, 2026 expiry, and saved in Netlify as production-only secret `PADDLE_LIVE_API_KEY`. A client-side token named `DropInn live checkout` was created and saved in Netlify as production-only `PADDLE_LIVE_CLIENT_TOKEN`. The live price ID is saved there as production-only `PADDLE_LIVE_SUPPORTER_PRICE_ID`. The live webhook signing secret was saved separately in Netlify as production-only `PADDLE_LIVE_WEBHOOK_SECRET`; its masked ending was checked against Paddle after correcting an accidental API-key paste. Netlify published the corrected-secret deploy. The default payment link is empty. PayPal, Apple Pay, and Bancontact are selected under **Checkout > Checkout settings > Payment Methods**. Google Pay was selected by owner request, but Paddle refused to save the setting while the default payment link is empty; enable and save it once the approved domain and default link are in place. Payout settings have no bank/payment details completed. No live transaction has been created.
...
The new live notification destination `ntfset_01m3fejg98514evct97gaynwbc` is active at `https://playdropinn.com/.netlify/functions/paddle-live-webhook`, subscribing to `transaction.completed`, `transaction.updated`, `transaction.canceled`, `adjustment.created`, and `adjustment.updated`. Its signing secret is saved in production-only Netlify `PADDLE_LIVE_WEBHOOK_SECRET`; do not put it in chat or source control. The dedicated handler reads `PADDLE_LIVE_API_KEY` and `PADDLE_LIVE_WEBHOOK_SECRET` so the public sandbox checkout and its notification destination stay intact during preparation. The original sandbox webhook endpoint always uses the original `PADDLE_API_KEY` and `PADDLE_WEBHOOK_SECRET`, even after the environment selector switches to production. Do not change the sandbox destination itself. Live webhook delivery cannot be confirmed until Paddle sends a live notification; no live payment or simulation has been generated during this migration.
