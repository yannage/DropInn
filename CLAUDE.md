...
- Turns allow 30 seconds, resolve early when all active humans commit, and show a paced result sequence for up to ten seconds. Every seated human can skip; if all skip, the next choosing turn starts early. The timeout always advances the table even if someone disconnects.
