...
export function teammatesAt(room: AdventureRoom, userId: string, targetId: string) {
  return room.seats.filter(seat => seat.kind === 'human' && seat.actorId !== userId
    && room.commits[seat.actorId]?.targetId === targetId).map(seat => ({
...
export function rollSupport(room: AdventureRoom, userId: string, action: PlayerAction) {
  const partners = teammatesAt(room, userId, action.targetId).filter(mate => mate.token !== action.token);
