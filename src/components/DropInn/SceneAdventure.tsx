...
import { CollectionGoal } from './Collection';
...
export function SceneAdventure({ room, chat }: { room: AdventureRoom; chat: ReactNode }) {
  ...
  const compactEffect = branchOption ? 'Your route vote counts even if the roll misses.' : protect ? 'Block 2 · great release blocks 3' : selection?.token === 'spotlight' ? `On success: ${selection.proposal?.effect}` : selection ? `${selection.token === 'assist' ? helpEffect : selection.token === 'fight' && !scene.combat ? 'Progress · clear the way' : effects[selection.token as IllustratedToken]}${support?.total ? ` · +${support.total}` : ''}` : branchOpen ? 'Place Help on a route to review its cost.' : armedToken ? hint : 'Tap a target to see what needs doing.';
  ...
  return <main className={`di-theater ${bubbleVisible ? 'has-inspection-bubble' : ''}`} aria-label="Adventure table" data-completed={room.status === 'completed'} onKeyDown={event => { if (event.key === 'Escape' && !drawer && storyMode === 'collapsed') { if (inspected) { event.preventDefault(); dismissInspection(true); } else if (focus) { event.preventDefault(); backToScene(); } } }}>
    ...
    <section className={`di-scene-dock ${inspected ? 'is-inspecting' : ''} ${selection || focus ? 'has-action' : ''}`} aria-label="Your move" data-phase={room.phase} data-holding={holding}>
      {room.phase === 'reveal' && lastRound ? <><RoundSequence summary={lastRound} now={now} bypass={revealBypassed} skipped={skipVoted} skipping={skippingReveal} canVote={canSkipReveal} skipCount={revealVoters.filter(seat => room.revealSkips?.includes(seat.actorId)).length} playerCount={revealVoters.length} onSkip={skipRound}/>{room.outcomes.some(outcome => outcome.chapter === room.chapter) && <div className="di-party-keepsake">{participant?.keepsakes.includes(scene.keepsake) && <KeepsakeArtwork name={scene.keepsake}/>}<div><span>Chapter {room.chapter + 1} complete</span>{participant?.keepsakes.includes(scene.keepsake) && <><strong>{scene.keepsake}</strong>{room.collectionVersion === 1 && <CollectionGoal earned={1}/>}</>}</div></div>}</> : <>
