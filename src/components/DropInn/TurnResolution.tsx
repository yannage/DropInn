...
export function TurnResolution({ event, room, now, userId }: { event: StoryEvent; room: AdventureRoom; now: number; userId: string }) {
  ...
  return <div className={`di-turn-resolution ${event.success === false ? 'is-complication' : ''}`} data-beat={beat} data-chapter={!!showChapter}>
    ...
    <div className="di-resolution-math" aria-hidden="true">
      ...
      </> : <span className="di-resolution-guaranteed"><Shield size={27} /> {event.result?.protection ?? ''}<small>guaranteed protection</small></span>}
    </div>
    ...
    <span className="di-game-sr" role="status">{event.roll !== undefined ? `Rolled ${event.roll}, modifier ${sign(modifier)}, total ${event.roll + modifier}. ` : ''}{resultLine(event)}. {benefits.join('. ')}</span>
