...
2. Apply the existing migrations in order, including `supabase/migrations/202609190001_dropinn_v2.sql`, `202609190002_server_character_access.sql`, and `202609200001_hero_customization.sql` to your Supabase project **before deploying the client**. The character access migration grants the server read access to saved heroes without changing player ownership policies; the customization migration adds the appearance/equipment columns required by hero creation and saving. Back up live data before your deployment procedure. V2 uses separate tables and retains legacy rooms.
